...
## from 14.04 -> 16.04 LTS, 18.04 today has apt errors that fail the build
FROM ubuntu:16.04
# MAINTAINER Zod (@wzod)
...
RUN cd /tmp && \
  wget -O yara-v3.4.0.tar.gz "https://github.com/VirusTotal/yara/archive/v3.4.0.tar.gz" && \
  wget -O volatility-2.6.1.tar.gz "https://github.com/volatilityfoundation/volatility/archive/2.6.1.tar.gz" && \

# Verify hashes FIXME
##  echo -e "b28ad483d4c0baf4b0b6a9162fcb497f6a822294be8b88621f678c15ed433d3d  volatility-2.6.1.tar.gz\\n528571ff721364229f34f6d1ff0eedc3cd5a2a75bb94727dc6578c6efe3d618b  yara-v3.4.0.tar.gz" > sha256sum.out && \
##  sha256sum -c sha256sum.out && \

# If hashes OK, install Yara and prepare Volatility setup
  tar vxzf volatility-2.6.1.tar.gz && \
...
RUN groupadd -r nonroot && \
  ...
  mkdir /home/nonroot && \
  mv -vf /tmp/volatility-2.6.1 /home/nonroot && \

# Setup Volatility
  cd /home/nonroot/volatility-2.6.1 && \
  chmod +x vol.py && \
  ln -fs /home/nonroot/volatility-2.6.1/vol.py /usr/local/bin/ && \
...
ENV USER nonroot
WORKDIR /home/nonroot/volatility-2.6.1
